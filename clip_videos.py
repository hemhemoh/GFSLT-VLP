...
# Used to Get thee details of the already clipped videos
import csv
import os
...
# Path to the new CSV file for clips information
clips_csv_path = 'clips_subtitle_alignment_batchA.csv'

# Track the number of processed clips
clip_count = 0
...
with open(clips_csv_path, 'w', newline='', encoding='utf-8') as clips_csvfile:
    ...
    with open(csv_file_path, newline='', encoding='utf-8') as csvfile:
        ...
        for row in csvreader:
            ...
            # Check if the current clip already exists
            if os.path.exists(clip_output_path):
                print(f"Clip {clip_name} exists, adding to CSV.")
                # Write the existing clip to the CSV
                clips_csvwriter.writerow([os.path.basename(clip_output_path), start_time, end_time, subtitle])
                clip_count += 1  # Increment the clip count
            else:
                print(f"Clip {clip_name} does not exist, skipping.")

# Output the total number of existing clips processed
print(f'Total number of existing clips processed: {clip_count}')

print('All existing clips have been processed and logged in the CSV.')
